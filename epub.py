...
class Epub():
    ...
    def asFile(self, rebuild=False):
        ...
        file = os.path.join(self._temp_obj_file.name,
                            self.identifier() + ".epub")
        with zipfile.ZipFile(file, 'w') as archive:
            ...
            archive.write(str(mimetype), 'mimetype',
            ...
            try:
                with zipfile.ZipFile(self.book_path, 'r') as archive:
                    if "mimetype" not in [item.filename for item in archive.filelist]:
                        if report_errors:
                            self.report.warn(
                                "No 'mimetype' file in ZIP; this is not an EPUB: " + self.book_path)
                        return False
                    ...
                    if "META-INF/container.xml" not in [item.filename for item in archive.filelist]:
                        if report_errors:
                            self.report.warn(
                                "No 'META-INF/container.xml' file in ZIP; this is not an EPUB: " + self.book_path)
                        return False

            except zipfile.BadZipfile:
                if report_errors:
                    self.report.warn(
                        "The book is a file, but not a ZIP file. This is not an EPUB: " + self.book_path)
                return False
    ...
    def opf_path(self):
        ...
        if os.path.isdir(self.book_path):
            container = ElementTree.parse(os.path.join(
                self.book_path, "META-INF/container.xml")).getroot()
        ...
        rootfiles = container.findall(
            '{urn:oasis:names:tc:opendocument:xmlns:container}rootfiles')[0]
        rootfile = rootfiles.findall(
            '{urn:oasis:names:tc:opendocument:xmlns:container}rootfile')[0]
    ...
    def spine(self):
        ...
        for itemref in itemrefs:
            item = opf.xpath(
                "//opf:manifest/opf:item[@id = '{}']".format(itemref.attrib["idref"]), namespaces=ns)
            data = {}
    ...
    def metadata(self):
        ...
        if not self._metadata:
            ...
            opf_metadata = opf.findall(
                '{http://www.idpf.org/2007/opf}metadata')[0]
    ...
    def update_prefixes(self):
        ...
        xslt = Xslt(report=self.report,
                    stylesheet=os.path.join(
                        Xslt.xslt_dir, Epub.uid, "update-epub-prefixes.xsl"),
                    source=opf_path,
        ...
        html_paths = opf_element.xpath(
            "/*/*[local-name()='manifest']/*[@media-type='application/xhtml+xml']/@href")
        for html_relpath in html_paths:
            html_path = os.path.normpath(os.path.join(
                os.path.dirname(opf_path), html_relpath))
    ...
    def update_opf_properties(self):
        ...
        for item in manifest:
            ...
            if properties:
                ...
                ) + list(properties)  # add any preexisting properties
        ...
        for itemref in spine:
            ...
            # add more types here if there's more we want to be regarded as secondary content (for instance footnotes?)
            if epub_type in ["cover"]:
                itemref.attrib["linear"] = "no"

        serialized = '<?xml version="1.0" encoding="UTF-8"?>\n' + \
            ElementTree.tostring(
                opf_element, pretty_print=True).decode('utf-8')
    ...
    def html_to_nav(pipeline, source, target):
        xslt = Xslt(pipeline,
                    stylesheet=os.path.join(
                        Xslt.xslt_dir, Epub.uid, "html-to-nav.xsl"),
                    source=source,
                    target=target)
        return xslt
    ...
    def html_to_opf(pipeline, source, target):
        xslt = Xslt(pipeline,
                    stylesheet=os.path.join(
                        Xslt.xslt_dir, Epub.uid, "html-to-opf.xsl"),
                    source=source,
                    target=target)
        return xslt
    ...
    def from_html(pipeline, dir_in, dir_out):
        ...
        with open(temp_opf, "w") as opf:
            ...
            opf.write(
                "<package xmlns=\"http://www.idpf.org/2007/opf\" version=\"3.0\" xml:base=\"file://{}/\">\n".format(epub_dir))
            ...
            for root, dirs, files in os.walk(dir_out):
                files.sort()
                for f in files:
                    item_relative = os.path.relpath(
                        os.path.join(root, f), epub_dir)
                    file_extension = os.path.split(
                        item_relative)[-1].split(".")[-1].lower()
                    ...
                    opf.write("        <item media-type=\"{}\" id=\"item_{}\" href=\"{}\"/>\n".format(
                        media_type, itempos, item_relative))
                    itempos += 1
            opf.write("    </manifest>\n")
            opf.write("    <spine>\n")
            opf.write(
                "        <itemref idref=\"{}\" id=\"itemref_1\"/>\n".format(contentref))
            opf.write("    </spine>\n")
            opf.write("</package>\n")
        ...
        xslt = Epub.html_to_opf(
            pipeline, temp_opf, os.path.join(epub_dir, "package.opf"))
        if not xslt.success:
            return None

        # create dir_out/EPUB/nav.xhtml based on input html (xslt)
        xslt = Epub.html_to_nav(pipeline, html_file,
                                os.path.join(epub_dir, "nav.xhtml"))
        if not xslt.success:
            return None
    ...
    def fix_permissions(self):
        ...
        # make sure the permissions is included in the zipped version
        self.asFile(rebuild=True)
